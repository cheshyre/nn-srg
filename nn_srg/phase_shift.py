...
def compute_phase_shifts_coupled_channel(pot: CoupledPotential):
    ...
    K = compute_K_matrix_coupled(V00.without_weights(), V01.without_weights(), V10.without_weights(), V11.without_weights(), Nrows, np.max(pot.nodes) + 1.0, pot.nodes, pot.weights)

    ps = [compute_phase_shifts_coupled_single_point(K, i, pot.nodes, Nrows) for i in range(Nrows)]
