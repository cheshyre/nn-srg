...
for i, lll in enumerate(lambdas):
    ...
    # ax.text(4.6, 4.5, "$\\lambda={}$".format(lam_disp), ha="right")
...
for i, lll in enumerate(lambdas):
    ...
    # Set xlabel on 3rd grid
    if i == 2:
        ax.set_xlabel(r"$p$ (fm$^{-1}$)", labelpad=102)
        ax.xaxis.set_label_position("top")
    ...
    # Add label for lambda value
    ax.text(2.5, 5.8, "$\\lambda={}$".format(lam_disp), ha="center")
    # if i != 0:
    #     ax.text(0.4, 4.5, "$\\lambda={}$".format(lam_disp))
    # else:
    #     ax.text(0.4, 4.5, "$\\lambda={}$".format(lam_disp),
    #         bbox=dict(facecolor="white", edgecolor="none", alpha=0.3, boxstyle="square,pad=0.1"),
    #     )
    # ax.text(4.6, 4.5, "$\\lambda={}$".format(lam_disp), ha="right")
